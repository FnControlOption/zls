...
pub const Type = struct {
    ...
    const EitherBuilder = struct {
        analyser: *Analyser,
        /// null when builder is empty or only contains compile errors
        has_type_val: ?bool,
        descriptors: Type.ArrayMap(NodeWithHandle),

        // Note that we don't hash/equate descriptors to remove duplicates
        ...
        fn deinit(builder: *EitherBuilder) void {
            builder.descriptors.deinit(builder.analyser.gpa);
        }
        ...
        fn add(builder: *EitherBuilder, ty: Type, descriptor: NodeWithHandle) !void {
            ...
            try builder.descriptors.put(builder.analyser.gpa, ty, descriptor);
        }

        fn resolve(builder: EitherBuilder) !?Type {
            const types = builder.descriptors.keys();
            ...
            for (types[1..]) |candidate| {
                chosen = try builder.analyser.resolvePeerTypes(chosen, candidate) orelse break;
            } else {
                return chosen;
            }

            const entries = try builder.analyser.arena.alloc(Type.Data.EitherEntry, types.len);
            const descriptors = builder.descriptors.values();
            for (entries, types, descriptors) |*entry, ty, descriptor| {
                entry.* = .{ .type_data = ty.data, .descriptor = descriptor };
            }

            const is_type_val = builder.has_type_val orelse false;
            return .{ .data = .{ .either = entries }, .is_type_val = is_type_val };
        }
    };
